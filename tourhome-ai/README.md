...
Free trial = **one published listing** per account; paid plans are ₮49,999 / ₮99,999 / ₮299,999 for 1, 3 and 12 months (`PLANS` in `src/lib/constants.ts`).

Enforcement lives in Postgres, not the UI:

- `enforce_publish_quota` — a BEFORE INSERT/UPDATE trigger on `properties` that rejects a transition to `published` when the trial is spent and no subscription is active. Raises `TOURHOME_TRIAL_USED`, which `readableDbError` turns into Mongolian copy.
...
- `supabase/schema.sql` parsed with libpg_query (pglast) → **138 statements, valid PostgreSQL**
